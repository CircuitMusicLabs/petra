...
def build_projects_for_dir(path)
  ...
  elsif File.directory?("#{path}/#{filename}") && filename != ".git" && filename != "_build" && filename != "packages" && filename != "." && filename != ".." && filename != "build" && filename != "sysbuild" && filename != "c74support" && !filename.match(/.*\.xcodeproj/)
...
puts "Finished building grainlabs objects."
puts
