...
typedef struct _cmlivecloud {
	...
	cm_cloud *grainmem; // struct array for storing the grains and associated variables in memory
	long cloudsize;
} t_cmlivecloud;
...
void *cmlivecloud_new(t_symbol *s, long argc, t_atom *argv) {
	...
	x->grainmem = (cm_cloud *)sysmem_newptrclear((x->cloudsize) * sizeof(cm_cloud));
	if (x->grainmem == NULL) {
		object_error((t_object *)x, "out of memory");
		return NULL;
	}
		...
		if (trigger && x->grains_count < x->grains_limit && !x->limit_modified && !x->recordflag && !x->buffer_modified && w_sample) {
			...
			for (readpos = 0; readpos < smp_length; readpos++) {
				...
				if (x->attr_sinterp) {
					...
					x->grainmem[slot].left[readpos] = ((x->ringbuffer[index] * w_read) * pan_left) * gain;
					x->grainmem[slot].right[readpos] = ((x->ringbuffer[index] * w_read) * pan_right) * gain;
				}
			}
		}
		/************************************************************************************************************************/
...
void cmlivecloud_cloudsize(t_cmlivecloud *x, t_symbol *s, long ac, t_atom *av) {
	long arg;
	arg = atom_getlong(av);
	if (arg < 1 || arg > x->cloudsize) {
		object_error((t_object *)x, "value must be in the range 1 - %d", x->cloudsize);
	}
	else {
